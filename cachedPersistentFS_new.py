...
def exit_gracefully(memory):
    lrucache.dumpdata()
    if os.path.exists('./.fileSystemstate.txt'):
        os.remove('./.fileSystemstate.txt') 
    f = open('./.fileSystemstate.txt','wb')
...
class Memory(LoggingMixIn, Operations):
    ...
    def create(self, path, mode):
        ...
        lrucache.set(node.children[tail].datakey,cData)
        #print("###########Cache write #############")
        self.fd += 1
        return self.fd
    ...
    def read(self, path, size, offset, fh):
        node = self.lookup(path)
        head, tail = os.path.split(path)
        if lrucache.get(node.datakey) != -1 :
            curData1 = lrucache.get(node.datakey)
            #print("###########Cache Hit #############")
            #print(curData1)
        else:
            #print("###########Cache Miss #############")
            ...
            lrucache.set(node.datakey,curData1)
    ...
    def symlink(self, target, source):
        ...
        db.files.insert_one(record)
        lrucache.set(node.datakey,curData)
        #print("###########Cache write #############")
...
if __name__ == '__main__':
    ...
    try:
        ...
        if os.path.exists('./.fileSystemstate.txt'):
            f1 = open('./.fileSystemstate.txt','rb')            
