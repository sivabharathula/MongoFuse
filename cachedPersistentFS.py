...
def exit_gracefully(memory):
    if os.path.exists('./.fileSystemstate1.txt'):
        os.remove('./.fileSystemstate1.txt') 
    f = open('./.fileSystemstate1.txt','wb')
...
class Memory(LoggingMixIn, Operations):
    ...
    def create(self, path, mode):
        ...
        self.lrucache.set(node.children[tail].datakey,cData)
        print("###########Cache write #############")
        self.fd += 1
        return self.fd
    ...
    def read(self, path, size, offset, fh):
        node = self.lookup(path)
        head, tail = os.path.split(path)
        if self.lrucache.get(node.datakey) != -1 :
            curData1 = self.lrucache.get(node.datakey)
            print("###########Cache Hit #############")
        else:
            print("###########Cache Miss #############")
            rv1 = db.files.find_one({"name": node.datakey})
            curData1 = pickle.loads(rv1["data"])
            self.lrucache.set(node.datakey,curData1)
    ...
    def symlink(self, target, source):
        ...
        db.files.insert_one(record)
        self.lrucache.set(node.datakey,curData)
        print("###########Cache write #############")
...
if __name__ == '__main__':
    if len(argv) != 2:
        ...
        exit(1)
    ccache = LRUCache(5)
    memory = Memory(ccache)
    try:
        logging.getLogger().setLevel(logging.DEBUG)
        if os.path.exists('./.fileSystemstate1.txt'):
            f1 = open('./.fileSystemstate1.txt','rb')            
